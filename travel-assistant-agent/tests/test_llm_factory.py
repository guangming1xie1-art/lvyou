...
class TestLLMProvider:
    ...
    def test_is_configured(self):
        ...
        with patch.dict(os.environ, {"DEEPSEEK_API_KEY": "test_key"}, clear=False):
            assert LLMProvider.DEEPSEEK.is_configured is True

        with patch.dict(os.environ, {"DEEPSEEK_API_KEY": ""}, clear=False):
            assert LLMProvider.DEEPSEEK.is_configured is False
...
class TestMODELCONFIGS:
    ...
    def test_all_tiers_have_models(self):
        ...
        standard_models = [
            m for m in MODEL_CONFIGS.values() if m.tier == ModelTier.STANDARD
        ]
...
class TestLLMFactory:
    ...
    def test_create_llm_with_custom_params(self):
        ...
        llm = LLMFactory.create_llm(
            "deepseek", ModelTier.CHEAP, temperature=0.5, max_tokens=2048
        )
    ...
    def test_get_tier_for_provider(self):
        ...
        assert (
            LLMFactory.get_tier_for_provider("dashscope") == ModelTier.CHEAP
        )  # 有多个层级，返回第一个
        assert LLMFactory.get_tier_for_provider("anthropic") == ModelTier.POWER

        # 测试通过模型名称查找
        assert LLMFactory.get_tier_for_provider("deepseek-chat") == ModelTier.CHEAP
        assert LLMFactory.get_tier_for_provider("qwen-turbo") == ModelTier.STANDARD
        assert (
            LLMFactory.get_tier_for_provider("claude-3-5-sonnet-20241022")
            == ModelTier.POWER
        )
    ...
    def test_is_provider_available(self):
        ...
        # DeepSeek 已配置
        assert LLMFactory.is_provider_available("deepseek")  is True
...
class TestLLMIntegration:
    ...
    def test_cheap_llm_has_different_cost_than_power(self):
        ...
        # 便宜层应该比强力层便宜
        cheap_avg_cost = sum(c.cost_per_1k_tokens for c in cheap_configs) / len(
            cheap_configs
        )
        power_avg_cost = sum(c.cost_per_1k_tokens for c in power_configs) / len(
            power_configs
        )
