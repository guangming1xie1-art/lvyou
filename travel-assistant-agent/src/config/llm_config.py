...
class LLMFactory:
    ...
    def create_llm(
        ...
        **kwargs
        ...
        if model_config is None:
            app_logger.warning(f"No model config found for provider: {provider}, tier: {tier}")
            # 使用默认配置创建
        ...
        if not api_key:
            app_logger.warning(f"API key not configured for provider: {model_config.provider.value}")
            # 尝试使用备选 provider
        ...
        # 创建 LLM 实例
        return cls._create_llm_instance(model_config, api_key, temperature, max_tokens, **kwargs)
    
    @classmethod
    def _find_model_config(cls, provider: str, tier: Optional[ModelTier]) -> Optional[ModelConfig]:
        """查找模型配置"""
    ...
    def _create_fallback_llm(
        ...
        """创建降级 LLM 实例"""
        app_logger.info(f"Creating fallback LLM for provider: {provider}")
